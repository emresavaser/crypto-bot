...
# Time-based exit integration (v4.8c upgrade)
try:
    from execution.time_exit import should_time_exit, calculate_time_decay_factor, TimeExitConfig
    TIME_EXIT_AVAILABLE = True
except ImportError:
    should_time_exit = None
    calculate_time_decay_factor = None
    TimeExitConfig = None
    TIME_EXIT_AVAILABLE = False
...
def _calculate_vol_adjusted_callback(
    cfg,
    atr_pct: float,
    base_callback: float,
) -> float:
    """
    Calculate volatility-adjusted trailing callback rate.

    Higher ATR -> wider callback (give more room)
    Lower ATR -> tighter callback (lock in profits faster)

    Args:
        cfg: Config object
        atr_pct: Current ATR as percentage of price
        base_callback: Base callback rate from config

    Returns:
        Adjusted callback rate
    """
    if not bool(getattr(cfg, "VOLATILITY_ADJUSTED_TRAILING", False)):
        return base_callback

    # Reference ATR (1% = 1x multiplier)
    ref_atr_pct = float(getattr(cfg, "TRAILING_ATR_REFERENCE_PCT", 0.01) or 0.01)
    min_mult = float(getattr(cfg, "TRAILING_VOL_MULT_MIN", 0.5) or 0.5)
    max_mult = float(getattr(cfg, "TRAILING_VOL_MULT_MAX", 2.0) or 2.0)

    if ref_atr_pct <= 0:
        return base_callback

    # Calculate multiplier based on ATR ratio
    vol_mult = atr_pct / ref_atr_pct
    vol_mult = _clamp(vol_mult, min_mult, max_mult)

    adjusted = base_callback * vol_mult
    # Ensure it stays within Binance limits (0.1% to 5%)
    return _clamp(adjusted, 0.1, 5.0)
...
async def handle_exit(bot, order: dict):
    ...
    async with lock:
        ...
        # === VOLATILITY-ADJUSTED TRAILING (v4.8c) ===
        # Calculate ATR percentage for volatility adjustment
        atr_pct = stop_pct  # stop_pct is already atr * mult / price

        cb_main_base = _clamp(float(cfg.TRAILING_CALLBACK_RATE), 0.1, 5.0)
        cb_tight_base = _clamp(float(cfg.TRAILING_TIGHT_PCT), 0.1, 5.0)
        cb_loose_base = _clamp(float(cfg.TRAILING_LOOSE_PCT), 0.1, 5.0)

        # Apply volatility adjustment if enabled
        cb_main = _calculate_vol_adjusted_callback(cfg, atr_pct, cb_main_base)
        cb_tight = _calculate_vol_adjusted_callback(cfg, atr_pct, cb_tight_base)
        cb_loose = _calculate_vol_adjusted_callback(cfg, atr_pct, cb_loose_base)
...
# ----------------------------
# Time-based exit checking (v4.8c)
# ----------------------------

async def check_time_based_exits(bot) -> int:
    """
    Check all positions for time-based exit conditions.
    Called by guardian loop.

    Returns:
        Number of positions closed due to time
    """
    if not TIME_EXIT_AVAILABLE or should_time_exit is None:
        return 0

    cfg = bot.cfg
    if not bool(getattr(cfg, "TIME_EXIT_ENABLED", True)):
        return 0

    closed_count = 0
    positions = dict(getattr(bot.state, "positions", {}) or {})
    ...
    for k, pos in positions.items():
        ...
        try:
            # Build time exit config from bot config
            time_cfg = TimeExitConfig(
                enabled=bool(getattr(cfg, "TIME_EXIT_ENABLED", True)),
                max_holding_minutes=float(getattr(cfg, "MAX_HOLDING_MINUTES", 240.0)),
                warning_minutes=float(getattr(cfg, "TIME_EXIT_WARNING_MINUTES", 180.0)),
                decay_start_pct=float(getattr(cfg, "TIME_DECAY_START_PCT", 0.5)),
            )

            should_exit, reason = should_time_exit(pos, time_cfg)

            if should_exit:
                # Get position details
                sym_raw = _resolve_raw_symbol(bot, k)
                remaining = abs(_safe_float(getattr(pos, "size", 0.0), 0.0))
                pos_side = _position_side_for_trade(getattr(pos, "side", "")) if _hedge_enabled(bot) else None
                ...
                try:
                    ...
                    await _safe_speak(
                        bot,
                        f"TIME EXIT {pos.side.upper()} {k}\n{reason}\nMax holding time exceeded",
                        "critical",
                    )
                    closed_count += 1

                except Exception as e:
                    log_entry.error(f"Time exit execution failed {k}: {e}")

        except Exception as e:
            log_entry.warning(f"Time exit check failed {k}: {e}")

    return closed_count


def _resolve_raw_symbol(bot, k: str) -> str:
    """Resolve canonical key to raw exchange symbol."""
    try:
        data = getattr(bot, "data", None)
