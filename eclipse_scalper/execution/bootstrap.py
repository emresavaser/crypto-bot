...
async def _init_exchange(cfg: Any) -> Any:
    ...
    # Check for demo or testnet mode
    use_demo = os.getenv("BINANCE_DEMO", "").strip() == "1"
    use_testnet = os.getenv("BINANCE_TESTNET", "").strip() == "1"

    # For Binance Futures, use binanceusdm exchange class
    if ex_name == "binance" and default_type == "future":
        ex_cls = getattr(ccxt, "binanceusdm", ex_cls)
        log_core.info("[bootstrap] Using binanceusdm for Futures")

    exchange = ex_cls(params)

    # Enable demo mode (uses demo-fapi.binance.com for PRIVATE endpoints only)
    # Public data (OHLCV, ticker) stays on mainnet for real market data
    if use_demo:
        try:
            if hasattr(exchange, 'urls') and 'demo' in exchange.urls:
                demo_urls = exchange.urls.get('demo', {})
                # Only set demo URLs for PRIVATE endpoints (trading)
                # Keep PUBLIC endpoints on mainnet for real market data
                for key in list(exchange.urls['api'].keys()):
                    if 'Private' in key and key in demo_urls:
                        exchange.urls['api'][key] = demo_urls[key]
                    elif 'sapi' in key.lower():
                        del exchange.urls['api'][key]
                log_core.info("[bootstrap] DEMO TRADING mode ENABLED (private=demo, public=mainnet)")
            else:
                log_core.warning("[bootstrap] Demo URLs not available in ccxt")
        except Exception as e:
            log_core.warning(f"[bootstrap] demo mode setup failed: {e}")
    elif use_testnet:
        try:
            exchange.set_sandbox_mode(True)
            log_core.info("[bootstrap] TESTNET/SANDBOX mode ENABLED")
        except Exception as e:
            log_core.warning(f"[bootstrap] set_sandbox_mode failed: {e}")
    ...
    try:
        ...
        log_core.warning(f"[bootstrap] load_markets failed: {e}")
        # In demo mode, try loading markets without API key (public endpoint)
        if use_demo:
            try:
                public_ex = ccxt.binanceusdm({'enableRateLimit': True})
                await public_ex.load_markets()
                exchange.markets = public_ex.markets
                exchange.markets_by_id = public_ex.markets_by_id
                exchange.symbols = public_ex.symbols
                exchange.ids = public_ex.ids
                await public_ex.close()
                log_core.info(f"[bootstrap] load_markets via public endpoint: {len(exchange.markets)} markets")
            except Exception as e2:
                log_core.warning(f"[bootstrap] public load_markets also failed: {e2}")
