...
try:
    from execution.heartbeat import get_heartbeat_monitor, beat as heartbeat_beat
    HEARTBEAT_AVAILABLE = True
except ImportError:
    get_heartbeat_monitor = None
    heartbeat_beat = None
    HEARTBEAT_AVAILABLE = False

# Distributed locking
try:
    from execution.distributed_lock import (
        get_lock_manager,
...
# Time-based exit checking
try:
    from execution.exit import check_time_based_exits
    TIME_EXIT_AVAILABLE = True
except ImportError:
    check_time_based_exits = None
...
async def guardian_loop(bot):
    ...
    """
    # === DISTRIBUTED LOCK CHECK (v1.8) ===
    if DISTRIBUTED_LOCK_AVAILABLE and callable(acquire_instance_lock):
        lock_enabled = bool(_cfg(bot, "DISTRIBUTED_LOCK_ENABLED", False))
        if lock_enabled:
            try:
                if not acquire_instance_lock():
                    log_core.critical("GUARDIAN: INSTANCE LOCK FAILED — another instance may be running")
                    log_core.critical("GUARDIAN: Aborting startup to prevent duplicate trading")
                    # Set shutdown event to prevent bot from running
                    _ensure_shutdown_event(bot).set()
                    _record_shutdown_reason(bot, "Instance lock failed", "guardian.distributed_lock")
                    return
                else:
                    log_core.info("GUARDIAN: Instance lock acquired successfully")
            except Exception as e:
                log_core.warning(f"GUARDIAN: Instance lock check error (continuing): {e}")

    # === HEARTBEAT INITIALIZATION (v1.8) ===
    if HEARTBEAT_AVAILABLE and callable(get_heartbeat_monitor):
        try:
            monitor = get_heartbeat_monitor()
            guardian_interval = float(_cfg(bot, "HEARTBEAT_GUARDIAN_SEC", 15.0))
            monitor.register_component("guardian", guardian_interval)
            log_core.info("GUARDIAN: Heartbeat monitoring initialized")
        except Exception as e:
            log_core.warning(f"GUARDIAN: Heartbeat init error: {e}")
    ...
    async def _one_cycle():
        ...
        # === HEARTBEAT TRACKING (v1.8) ===
        if HEARTBEAT_AVAILABLE and callable(heartbeat_beat):
            try:
                heartbeat_beat("guardian")
            except Exception:
                pass
        ...
        # === TIME-BASED EXIT CHECK (v1.8) ===
        if TIME_EXIT_AVAILABLE and callable(check_time_based_exits):
            time_exit_enabled = bool(_cfg(bot, "TIME_EXIT_ENABLED", True))
            if time_exit_enabled:
                try:
                    closed_count = await check_time_based_exits(bot)
                    if closed_count > 0:
                        log_entry.info(f"GUARDIAN: Time-based exits triggered: {closed_count} positions")
                except Exception as e:
                    log_entry.warning(f"GUARDIAN: Time exit check error: {e}")

        # === HEARTBEAT ALERT CHECK (v1.8) ===
        if HEARTBEAT_AVAILABLE and callable(get_heartbeat_monitor):
            try:
                monitor = get_heartbeat_monitor()
                ...
                log_entry.debug(f"GUARDIAN: Heartbeat alert check error: {e}")
