...
# Slippage and liquidity checks (v4.8c upgrade)
try:
    from execution.slippage_estimator import check_slippage_before_entry, SlippageEstimatorConfig
    SLIPPAGE_ESTIMATOR_AVAILABLE = True
except ImportError:
    check_slippage_before_entry = None
    SlippageEstimatorConfig = None
    SLIPPAGE_ESTIMATOR_AVAILABLE = False
...
async def try_enter(bot, sym: str, side: str):
    ...
    async with lock:
        ...
        # === PRE-ENTRY SLIPPAGE AND LIQUIDITY CHECK (v4.8c) ===
        slippage_check_enabled = bool(getattr(cfg, "SLIPPAGE_PRE_CHECK_ENABLED", True))
        liquidity_check_enabled = bool(getattr(cfg, "LIQUIDITY_CHECK_ENABLED", True))

        if SLIPPAGE_ESTIMATOR_AVAILABLE and (slippage_check_enabled or liquidity_check_enabled):
            try:
                # Estimate position size for slippage calculation
                preliminary_risk = equity * float(cfg.MAX_RISK_PER_TRADE)
                preliminary_notional = preliminary_risk / 0.02  # rough estimate

                should_enter, slip_reason = await check_slippage_before_entry(
                    bot=bot,
                    symbol=sym_raw,
                    side=side,
                    amount_usdt=preliminary_notional,
                    max_slippage_pct=float(cfg.SLIPPAGE_MAX_PCT),
                    max_liquidity_impact_pct=float(getattr(cfg, "LIQUIDITY_MAX_IMPACT_PCT", 0.10)),
                )

                if not should_enter:
                    _skip(bot, k, side, f"slippage/liquidity check: {slip_reason}")
                    return

            except Exception as e:
                # Non-blocking: log warning but continue
                log_entry.warning(f"{k} slippage check error (continuing): {e}")
