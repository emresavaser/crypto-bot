...
class ColorFormatter(logging.Formatter):
    ...
    def __init__(self, *, use_color: bool = True, utc: bool = False):
        ...
        if self.use_color:
            self.formats = {
                logging.DEBUG: f"%(asctime)s | * | {self.grey}%(levelname)-8s{self.reset} | %(name)-12s | %(message)s",
                logging.INFO: f"%(asctime)s | * | {self.green}%(levelname)-8s{self.reset} | %(name)-12s | %(message)s",
                logging.WARNING: f"%(asctime)s | * | {self.yellow}%(levelname)-8s{self.reset} | %(name)-12s | %(message)s",
                logging.ERROR: f"%(asctime)s | * | {self.red}%(levelname)-8s{self.reset} | %(name)-12s | %(message)s",
                logging.CRITICAL: f"%(asctime)s | * | {self.bold_red}%(levelname)-8s{self.reset} | %(name)-12s | %(message)s",
            }
        else:
            self.formats = {
                logging.DEBUG: "%(asctime)s | * | %(levelname)-8s | %(name)-12s | %(message)s",
                logging.INFO: "%(asctime)s | * | %(levelname)-8s | %(name)-12s | %(message)s",
                logging.WARNING: "%(asctime)s | * | %(levelname)-8s | %(name)-12s | %(message)s",
                logging.ERROR: "%(asctime)s | * | %(levelname)-8s | %(name)-12s | %(message)s",
                logging.CRITICAL: "%(asctime)s | * | %(levelname)-8s | %(name)-12s | %(message)s",
            }
