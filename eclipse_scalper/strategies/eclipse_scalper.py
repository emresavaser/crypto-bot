...
# === v3.1 UPGRADES: Order Flow + ML Integration ===

# Order flow analysis (optional)
try:
    from strategies.order_flow import get_order_flow_analyzer, OrderFlowConfig
    ORDER_FLOW_AVAILABLE = True
except ImportError:
    get_order_flow_analyzer = None
    OrderFlowConfig = None
    ORDER_FLOW_AVAILABLE = False

# ML signal filtering (optional)
try:
    from strategies.ml_predictor import filter_signal_with_ml, get_ml_filter, MLConfig
    from strategies.ml_features import extract_all_features, compute_technical_features
    ML_AVAILABLE = True
except ImportError:
    filter_signal_with_ml = None
    get_ml_filter = None
    MLConfig = None
...
def scalper_signal(sym: str, data=None, cfg=None, *args, **kwargs) -> tuple[bool, bool, float]:
    ...
    try:
        ...
        # === ORDER FLOW ANALYSIS (v3.1) ===
        order_flow_enabled = bool(getattr(cfg, "ORDER_FLOW_ENABLED", False) if cfg else False)
        order_flow_signal_strength = 0.0
        order_flow_bias = ""

        if ORDER_FLOW_AVAILABLE and order_flow_enabled and callable(get_order_flow_analyzer):
            try:
                analyzer = get_order_flow_analyzer()
                if analyzer is not None:
                    of_strength, of_bias = analyzer.get_order_flow_signal(k)
                    order_flow_signal_strength = float(of_strength)
                    order_flow_bias = str(of_bias)

                    # Add order flow votes
                    if order_flow_signal_strength > 0.3:
                        # Strong order flow signal
                        votes += 1.5 * order_flow_signal_strength
                        max_votes += 1.5

                        # Bias alignment bonus
                        if (of_bias == "bullish" and long_mom) or (of_bias == "bearish" and short_mom):
                            votes += 1.0
                        max_votes += 1.0

                        # CVD confirmation
                        cvd_signal = analyzer.calculate_cvd_signal(k)
                        if abs(cvd_signal) > 0.2:
                            if (cvd_signal > 0 and long_mom) or (cvd_signal < 0 and short_mom):
                                votes += 0.8
                        max_votes += 0.8
                    else:
                        max_votes += 3.3  # Account for potential order flow votes
            except Exception as e:
                _diag_throttled(f"{k}:orderflow", f"SIGNAL: Order flow error {k}: {e}", every_sec=30.0)
        ...
        # === ML SIGNAL FILTERING (v3.1) ===
        ml_enabled = bool(getattr(cfg, "ML_ENABLED", False) if cfg else False)
        ml_confidence = confidence  # Start with rule-based confidence

        if ML_AVAILABLE and ml_enabled and (long_signal or short_signal) and callable(filter_signal_with_ml):
            try:
                # Build features dict for ML
                ml_features = {}
                if callable(compute_technical_features):
                    try:
                        ml_features = compute_technical_features(df)
                    except Exception:
                        pass

                # Add computed features
                ml_features["momentum"] = float(mom)
                ml_features["rsi_14"] = float(rsi)
                ml_features["rsi_norm"] = float(rsi) / 100.0
                ml_features["adx"] = float(adx)
                ml_features["bb_width"] = float(bb_width_pct)
                ml_features["volume_z"] = float(vol_z)
                ml_features["vwap_distance"] = float(vwap_distance)
                ml_features["order_imbalance"] = float(order_flow_signal_strength) if ORDER_FLOW_AVAILABLE else 0.0

                # Apply ML filtering
                ml_long, ml_short, ensemble_conf = filter_signal_with_ml(
                ...
                # ML can filter out signals but not create them
                if long_signal and not ml_long:
                    _diag_throttled(
                        f"{k}:mlfilter",
                        f"SIGNAL: ML filtered LONG {k} (ensemble={ensemble_conf:.2f})",
                        every_sec=10.0,
                    )
                    long_signal = False

                if short_signal and not ml_short:
                    _diag_throttled(
                        f"{k}:mlfilter",
                        f"SIGNAL: ML filtered SHORT {k} (ensemble={ensemble_conf:.2f})",
                        every_sec=10.0,
                    )
                    short_signal = False

                # Use ensemble confidence
                ml_confidence = ensemble_conf

            except Exception as e:
                _diag_throttled(f"{k}:mlerror", f"SIGNAL: ML error {k}: {e}", every_sec=30.0)

        # Use ML confidence if available, else rule-based
        final_confidence = ml_confidence if ml_enabled else confidence

        if long_signal or short_signal:
            direction = "LONG" if long_signal else "SHORT"
            div_flag = (bullish_div or bearish_div or hidden_bullish or hidden_bearish)
            of_info = f"OF={order_flow_signal_strength:.2f}/{order_flow_bias}" if ORDER_FLOW_AVAILABLE and order_flow_enabled else ""
            ml_info = f"ML={ml_enabled}" if ML_AVAILABLE else ""
            log.critical(
                f"SIGNAL {direction} {k} | conf={final_confidence:.2f} | micro={micro_profile}\n"
                ...
                f"move={'HARD' if vol_spike_hard else ('SOFT' if vol_spike_soft else ('ATR' if impulse_soft else 'NO'))} {of_info} {ml_info}"
            )

        return bool(long_signal), bool(short_signal), float(final_confidence)
